...
# Runs a command inside the container to pull another container image (quay.io/rbrhssa/chbox) using Podman and stores it in a specific root directory (/usr/lib/containers/storage). 
...
# Creates a directory /usr/etc-system and then echoes some SSH configuration into a file 30-auth-system.conf. It also adds an SSH public key into another file root.keys.
